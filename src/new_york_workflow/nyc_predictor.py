"""
NYC Airbnb price prediction — production predictor.

Loads all artifacts from models/nyc/, applies the identical feature
engineering pipeline used during training, and returns a dollar price.

Usage:
    predictor = NYCAirbnbPredictor()
    result    = predictor.predict_raw(raw_dict)
...
BOROUGH_DUMMIES  = ["borough_Brooklyn", "borough_Manhattan", "borough_Queens", "borough_Staten Island"]


class NYCAirbnbPredictor:
    """Production-grade NYC Airbnb price predictor."""
    ...
    def __init__(self, model_dir: Path = MODEL_DIR):
        ...
        self.model               = _load("nyc_xgb_model.pkl")
        ...
        logger.info(
            "NYCAirbnbPredictor loaded — %d features, neighbourhood means for %d areas",
            len(self.feature_list), len(self.neighbourhood_means),
        )
    ...
    def predict_raw(self, raw: dict) -> dict:
        """
        Accept raw listing fields, run the full feature engineering pipeline,
        and return predicted price in dollars.

        Returns dict with: price_usd (float), price_str (str), log_price (float)
        """
        self._validate(raw)
        ...
        log_price  = float(self.model.predict(df_scaled)[0])
        price_usd  = float(np.expm1(log_price))

        logger.debug("predict_raw → log_price=%.4f  price=$%.2f", log_price, price_usd)
        return {
    ...
    def predict_batch(self, raws: list[dict]) -> list[dict]:
        """Batch predict — logs and skips invalid rows rather than failing the batch."""
        results = []
        ...
        logger.info("Batch complete — %d/%d succeeded", ok, len(raws))
        return results
    ...
    def model_info(self) -> dict:
        return {
            ...
            "mape_pct":       23.60,
            "training_rows":  16388,
            "price_cap_used": "$1,562/night (top 1% luxury listings excluded from training)",
        }
    ...
    def _engineer(self, raw: dict) -> dict:
        """Apply the identical FE pipeline from 2_feature_engineering.py."""
        r = {}
        ...
        # ── room type dummies ──────────────────────────────────────────────
        rt = raw["room_type"]
        r["room_type_Entire home/apt"] = int(rt == "Entire home/apt")
        r["room_type_Hotel room"]      = int(rt == "Hotel room")
        r["room_type_Private room"]    = int(rt == "Private room")
        r["room_type_Shared room"]     = int(rt == "Shared room")
        ...
        # ── borough dummies (Bronx = reference category, no dummy) ────────
        borough = raw["borough"]
        ...
        r["neighbourhood_price_rank"] = self.neighbourhood_means.get(
            neighbourhood, self.global_mean
        )
